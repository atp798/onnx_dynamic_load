...
ONNXTensorElementDataType TensorProtoDataTypeToONNXTensorElementDataType(
  ...
  switch (elem_type) {
    ...
    default:
      ...
      err_msg << "Tensor proto data type: '" << elem_type
              << "' convert failed.";
...
#define COPY_TENSOR_DATA(tensor, data_ptr, length, data_type)     \
    ...
    memcpy(tensor->data_type().data(),                            \
           data_ptr,                                              \
           length *                                               \
              sizeof(std::remove_reference<decltype(              \
                         tensor->data_type())>::type::value_type))

void SetTensorData(Tensor* tensor, void* data_ptr, size_t length) throw(std::invalid_argument){
...
const std::string OnnxApiWrapper::kOnnxruntimeLibName("onnxruntime_pybind11_state.so");
...
OnnxApiWrapperPtr OnnxApiWrapper::GetInstance() throw(std::runtime_error){
  // TODO: Here we don't use DCLP for the reason that,
...
void OnnxApiWrapper::checkStatus(OrtStatus* status) const throw(std::runtime_error) {
  if (status != NULL) {
...
bool OnnxApiWrapper::InferModel(
    const ModelProto mp,
  ...
  checkStatus(pOrt_->CreateSessionFromArray(
      env, mp_buff.data(), mp_buff.size(), session_options, &sess));

  OrtMemoryInfo* memory_info;
  checkStatus(pOrt_->CreateCpuMemoryInfo(
  ...
  for (size_t i = 0; i < input_num; ++i) {
    ...
    size_t data_len = 0;
    GetONNXTensorData(
        input_tensors[i], const_cast<const void**>(&data_ptr), &data_len);
  ...
  for (decltype(output_num) i = 0; i < output_num; ++i) {
    ...
    // set tensor data
    size_t data_len = std::accumulate(
        output_dims.begin(),
        output_dims.end(),
        (int64_t)1,
  ...
  // release resources
  // TODO: Use RAII theory to handle resources.
  for (decltype(output_num) i = 0; i < output_num; ++i) {
