...
int main(int argc, char* argv[]) {
  ...
  for(decltype(tensor_size) i=0;i<tensor_size;++i){
    float neg=((float)(rand()%997)/997.0)<0.5 ? -1.0:1.0;
    data.floats().push_back( ((float)(rand()%997)/997.0) * neg);
  }
  data.setName("node");
  ...
  std::vector<Dimension> dim_sizes{data.sizes().cbegin(), data.sizes().cend()};
  ...
  api_wrapper->InferModel(mp, input_names, input_tensors, 
